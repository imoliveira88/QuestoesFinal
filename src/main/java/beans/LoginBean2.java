...
@SessionScoped
public class LoginBean2 extends BeanGeral implements Serializable {
    ...
    public String login() throws ServletException,NullPointerException{
        ...
        try {
            ...
            if (valida(login, senha)) {
                ...
                addMensagem("Usuário ou senha inválidos!");
                return "login";
            }
            }else{
                setLogin(null);
                setUsuarioSessao(null);
                addMensagem("Captcha inválido!");
                return "login";    
            }
        } catch (Exception e) {
            setLogin(null);
            setUsuarioSessao(null);
            session.setAttribute("usuarioDaSessao", usuarioSessao);
            addMensagem("Senha ou usuário inválidos!");
            return "login";
        }
    }
